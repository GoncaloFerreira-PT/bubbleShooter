...
bool ResourceManager::Load()
{
    ...
    {
        for (auto &obj : jsonFile["textures"])
        {
    ...
    {
        for (auto &obj : jsonFile["atlases"])
        {
            if (!TextureManager::Instance().LoadAtlas(obj))
            {
