...
bool TextureManager::Load(const std::string &filePath,
  ...
  if (IsTextureInMap(textureID)) {
    Console::Warn("Texture with ID " + textureID +
  ...
  if (!texture) {
    Console::Error("Couldn't create static texture: " +
...
bool TextureManager::LoadAtlas(nlohmann::basic_json<> &atlasData) {
  ...
  if (!texture) {
    Console::Error("Couldn't create atlas texture: " +
                   std::string(SDL_GetError()));
    return false;
  }
  ...
  auto sharedTexture =
      std::shared_ptr<SDL_Texture>(texture, SDL_DestroyTexture);
  for (const auto &region : atlasData["regions"]) {
...
void TextureManager::Draw(const std::string &textureID, SDL_FRect rect,
                          SDL_Color modulate) {
...
void TextureManager::DrawRotated(const std::string &textureID, SDL_FRect rect,
                                 SDL_Color modulate, float angle,
  ...
  SDL_SetTextureColorMod(texture, modulate.r, modulate.g, modulate.b);
  SDL_SetTextureAlphaMod(texture, modulate.a);
  SDL_RenderTextureRotated(renderer, texture, srcRect, &rect, angle, &center,
                           flip);
}
...
std::shared_ptr<TextureManager::TextureData>
TextureManager::GetTextureData(const std::string &textureID) {
  auto it = textureMap.find(textureID);
  if (it != textureMap.end()) {
    return it->second;
  }

  Console::Warn("Texture with ID " + textureID + " not found in the map.");
  return nullptr;
}