...
struct AppState {
  ...
  BubbleGame *game = nullptr;
  Uint64 lastTime = 0;
};

SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
  Console::Log("Starting application: Bubble Shooter by Gonçalo Ferreira");
  if (!SDL_SetAppMetadata("Bubble Shooter by Gonçalo Ferreira", "1.0", "")) { return SDL_APP_FAILURE; }
  ...
  *appstate = appState;
  ...
  appState->game = new BubbleGame();

  if (!SDL_CreateWindowAndRenderer("Bubble Shooter by Gonçalo Ferreira", Core::Config::Window::WIDTH,
                                   Core::Config::Window::HEIGHT, SDL_WINDOW_RESIZABLE, &appState->window,
                                   &appState->renderer)) {
    Console::Error("Couldn't create window and renderer: " + std::string(SDL_GetError()));
    return SDL_APP_FAILURE;
  }
  SDL_SetRenderLogicalPresentation(appState->renderer, Core::Config::Window::WIDTH, Core::Config::Window::HEIGHT,
                                   SDL_LOGICAL_PRESENTATION_LETTERBOX);

  TextureManager::Instance().SetRenderer(appState->renderer);
  appState->game->Initialize(appState->renderer);
...
SDL_AppResult SDL_AppIterate(void *appstate) {
  ...
  appState->game->Update(deltaTime);
  appState->game->Draw(appState->renderer);
  return SDL_APP_CONTINUE;
}
...
SDL_AppResult SDL_AppEvent(void *appstate, SDL_Event *event) {
  AppState *appState = (AppState *)appstate;
  if (appState && appState->game) { appState->game->HandleInput(event); }
